...
4. **File it** with `fleet_file_ticket`, passing `title`, `body`, and optionally `priority` and `ready`. Leave `ready` at its default (true) unless the ticket needs human curation before a worker should pick it up.
...
- `fleet_file_ticket` — files a new ticket. Input: `{ title, body, priority?: "p1"|"p2"|"p3", ready?: boolean }`.
