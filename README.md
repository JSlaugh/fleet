...
- **Plan children default to the backlog.** Unless the project sets `planChildrenReady: true` (see [Epic decomposition](#epic-decomposition-fleetplan) below), a decomposed epic's child tickets are filed as `fleet:backlog` — visible in the board's Backlog column but not claimable until a human releases them, either all at once with **Accept plan** on the epic or one at a time with the card's **Ready** button. Tickets filed via the fleet-backlog skill work the other way: `ready` defaults to true (immediately pickable), and the filer passes `ready: false` to land in the backlog when a ticket needs human curation first.
...
Labeling an issue `fleet:plan` runs it as a read-only planning session instead of a coding one: the worker explores the repo for context but never writes files or commits, and its structured result is a list of self-contained, PR-sized child tickets (each with a title, body, optional priority, and an honest `light`/`standard`/`elevated` tier guess) rather than a diff. On completion, fleet files each child as its own issue — tagged with its suggested tier label and, if the project sets `planChildrenReady: true`, `fleet:ready` immediately; otherwise `fleet:backlog`, so it shows in the board's Backlog column without being claimable. The epic issue itself goes to `fleet:review` with the child list in its status comment and never opens a PR. Reviewing an epic means reviewing its plan: **Accept plan** in the dashboard releases every backlog child to `fleet:ready` (children with an unsatisfied `Depends-on` still wait in Ready until it clears; a child a human already moved past the backlog is left alone) and closes the epic — the epic's job was the decomposition, so it doesn't stay open tracking its children's delivery. If an epic is instead left in review, it auto-closes once every child is closed. A blocked decomposition works exactly like a blocked coding ticket (question posted, session held open for a reply).
...
In map form a `default` profile is required; every other key becomes a selectable profile. A profile can also be written as an object (`{ setup, teardown, contract, review, verify, tier }`) instead of a bare step list, to declare `teardown:` steps (below), a `contract:`/`review:` markdown appendix, `verify:` commands, and/or a default model `tier:` for tickets of that type (below). The daemon reads `fleet.yaml` from the **fresh worktree**, after `git worktree add` and before the worker session starts — provisioning stays fully deterministic and daemon-side, with no agent or model involvement.
...
**Per-worktree resources: `teardown:` and `FLEET_*` variables.** Setup and teardown steps run with `FLEET_ISSUE` (issue number), `FLEET_PROJECT` (project name), and `FLEET_WORKTREE` (absolute worktree path) in their environment, so a step can provision resources unique to its worktree — e.g. `docker compose -p ${FLEET_PROJECT}-wt-${FLEET_ISSUE} up -d`. An object-form profile's optional `teardown:` list (same `name`/`run`/`allowFailure` step shape as `setup:`) releases those resources when the worktree is discarded: cleanup after the PR merges/closes, an operator restart, or a re-claim replacing the previous attempt's worktree — and on daemon startup for any ticket whose worktree is already gone but whose teardown never ran (a crash between removal and teardown). Teardown is wholly best-effort: a failing step is logged and the remaining steps still run, never blocking the ticket. For untyped tickets the `default` profile's `teardown:` applies, mirroring the setup fallback. A `fleet.yaml` with no `teardown:` keys behaves exactly as before — nothing new runs anywhere.
...
Reasoning effort (`low`/`medium`/`high`/`xhigh`/`max`) is an orthogonal, cheaper lever than swapping models: it's selected the same way — `fleet:elevate` → `elevatedEffort`, `fleet:light` → `lightEffort`, otherwise the project's `effort` — but each tier is independently optional, so an unset field falls back to the Agent SDK's own default rather than to a sibling tier. Machine/plan review sessions prefer `lightEffort ?? effort`, matching how they already prefer the light model.
