...
description: Use when adding, renaming, or removing a field on FleetConfigSchema or ProjectConfigSchema in packages/shared/src/index.ts — three places must change together or pnpm test fails.
---

# config-shape-change

`fleet.config.json` (gitignored, per-machine) is validated against these zod schemas in `packages/shared/src/index.ts`:
...
1. **The schema** — `packages/shared/src/index.ts`. Add the field to `ProjectConfigSchema` (per-project) or `FleetConfigSchema` (top-level), with a `.default(...)` if it should be optional, or document why it's required.
