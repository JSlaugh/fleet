...
GitHub labels are the source of truth for ticket status; `.fleet/state.json` (`StateStore`, `store/state.ts`) is operational cache only — per-ticket (worktree paths, session IDs, cost, model usage, and the `lastReviewHandledAt`/`autoElevated`/`isPlan` fields the behavior above depends on) plus two daemon-wide fields, `pausedUntil` for the plan usage-limit pause and `paused` for operator drain mode (both gate claims/resumes via `isPaused` in `loop/pause.ts`) — and `.fleet/journals/<project>/<issue>.jsonl` (`Journal`, `store/journal.ts`) holds summarized session transcripts for the dashboard. On boot, `StateStore.clearLiveFlags()` reconciles orphaned `running` tickets to `stalled` since no sessions survive a restart. Cleanup (worktree + branch removal) only happens once the PR is merged/closed *and* the issue is closed, at which point the ticket's record moves from `state.json` into `.fleet/history.json` (`HistoryStore`, `store/history.ts`, capped at 50) so the dashboard's Done column can still show it.
